...
// Consolidated navigation component that replaces all the scattered nav components
export function ConsolidatedNavbar({
  ...
  // Minimal variant - just logo and essential controls
  if (variant === 'minimal') {
    return (
      <header className={cn("flex items-center justify-between py-2 px-4 border-b", className)}>
  ...
  // Sidebar variant - for layout sidebars
  if (variant === 'sidebar') {
    return (
  ...
  // Full variant - comprehensive navbar
  if (!mounted) {
...
export const SimpleNavbar = (props: ConsolidatedNavbarProps) =>
  <ConsolidatedNavbar {...props} variant="minimal" />;
export const SidebarNav = (props: ConsolidatedNavbarProps) =>
