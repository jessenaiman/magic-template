...
export default function TailwindTextPage() {
  const [animationSpeed, setAnimationSpeed] = useState<'normal' | 'fast'>(
    'normal'
  );
    ...
    <div className="space-y-6">
      <div className="space-y-2">
        <h2 className="text-2xl font-bold tracking-tight">
          TailwindCSS Text Utilities
        </h2>
        <p className="text-muted-foreground">
          Text styling and animations using TailwindCSS utility classes. These
          examples demonstrate how to achieve text effects using Tailwind's
      ...
      >
        {customization => (
          <div className="text-2xl font-semibold">
            <span
              className={`${gradientClasses[gradientStyle]} bg-clip-text text-transparent bg-300% animate-gradient`}
            >
              Tailwind Gradient
            </span>
          </div>
  ...
  Shiny Text
      ...
      >
        {customization => (
          <div
            className={`text-4xl font-bold text-gray-400 bg-clip-text bg-200% animate-shine ${animationSpeed === 'fast' ? 'animate-shine-fast' : ''}`}
          >
            Shiny Text
          </div>
        )}
