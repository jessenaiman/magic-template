...
export default function MagicButtonsPage() {
  ...
  return (
    <div className="grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
      ...
      >
        {customization => (
          <RainbowButton>
            {customization.buttonText || 'Get Unlimited Access'}
          </RainbowButton>
        )}
      </PreviewTile>
      {/* Ripple Button */}
      ...
      >
        {customization => (
          <RippleButton rippleColor={customization.rippleColor || '#ADD8E6'}>
  ...
  <div className="absolute inset-0 rounded-md opacity-0 transition-opacity duration-300 group-hover:opacity-100" style={{ background: \`linear-gradient(to right, \${gradientFrom}dd, \${gradientTo}dd)\` }}></div>
      ...
      >
        {customization => (
          <Button
            className="group relative inline-flex h-12 items-center justify-center rounded-md px-6 py-2 font-medium text-white shadow-lg transition-all duration-300 hover:shadow-xl hover:scale-110"
            style={{
              background: `linear-gradient(to right, ${customization.gradientFrom || '#8b5cf6'}, ${customization.gradientTo || '#ec4899'})`,
            }}
          >
            <span className="relative z-10 transition-transform duration-300 group-hover:scale-110">
            ...
            <div
              ...
              style={{
                background: `linear-gradient(to right, ${customization.gradientFrom || '#8b5cf6'}dd, ${customization.gradientTo || '#ec4899'}dd)`,
              }}
            ></div>
          </Button>
        )}
  ...
  <div className="absolute inset-0 rounded-md bg-gradient-to-r from-transparent via-white/10 to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100"></div>
      ...
      >
        {customization => (
          <Button
            className="group relative inline-flex h-12 items-center justify-center rounded-md px-6 py-2 font-medium text-white shadow-lg transition-all hover:shadow-xl"
            style={{
              backgroundColor: customization.spotlightBackground || '#0f172a',
            }}
          >
            <span className="relative z-10">
              {customization.buttonText || 'Spotlight Effect'}
            </span>
            <div className="absolute inset-0 rounded-md bg-gradient-to-r from-transparent via-white/10 to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100"></div>
          </Button>
        )}
