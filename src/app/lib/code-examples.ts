...
export function createCodeExamples(
  ...
  return examples.map(([code, language, title]) =>
    createCodeExample(code, language, title)
  );
}
...
export const buttonExamples = {
  basic: (className: string, children: string) =>
    createCodeExample(
  ...
  react: (className: string, children: string) =>
    createCodeExample(
      `<Button className="${className}">
  ${children}
</Button>`,
  ...
  css: (selector: string, styles: string) =>
    createCodeExample(
      `.${selector} {
${styles
  .split('\n')
...
export function getInstallCommand(
  library: 'tailwind' | 'shadcn' | 'magicui' | 'animate-ui'
): string {
  switch (library) {
    case 'tailwind':
