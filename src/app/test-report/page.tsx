...
function computeMetrics(suites: TestSuite[]): TestMetrics {
  ...
  return {
    ...
    coverage: Math.max(
      0,
      Math.min(100, Math.round((passedTests / Math.max(1, totalTests)) * 100))
    ),
    ...
    try {
      ...
      if (e2eRes.ok) {
        ...
        builtSuites.push({
          ...
          duration: 0,
          lastRun:
            e2eData.endTime || e2eData.startTime || new Date().toISOString(),
      ...
      <Tabs defaultValue="suites" className="space-y-6">
        ...
        <TabsContent value="suites" className="space-y-4">
          <div className="grid gap-4">
            {(suites.length ? suites : []).map(
              (suite: TestSuite, index: number) => (
                <Card
                  key={index}
                    ...
                    <div className="mt-4">
                      <div className="w-full bg-gray-200 rounded-full h-2">
                        <div
                          ...
                          style={{
                            width: `${(suite.testsPassed / suite.testsTotal) * 100}%`,
                          }}
                        />
                      </div>
                    </div>
          ...
          {selectedSuite ? (
                                  ...
                                  {sorted.map((testCase, idx) => (
                                    ...
                                    >
                                        ...
                                        <div className="flex items-center gap-2 shrink-0">
                                          ...
                                          <Button
                                            ...
                                            onClick={() => {
                                              ...
                                              const prompt =
                                                `Generate a detailed fix spec for the following failing test in Magic Template.\n\n` +
                                      ...
                                      {testCase.suggestions &&
                                        testCase.suggestions.length > 0 && (
                                          <div>
                                            <h5 className="font-medium mb-2">
                                              Suggestions
                                            </h5>
                                            <ul className="list-disc list-inside space-y-1 text-sm text-muted-foreground">
                                              {testCase.suggestions.map(
                                                (s, i) => (
                                                  <li key={i}>{s}</li>
                                                )
                                              )}
            ...
            <Card>
              <CardContent className="text-center py-8">
                ...
                {loading ? (
                  <p className="text-muted-foreground">
                    Loading live test results…
                  </p>
                ) : suites.length === 0 ? (
                  <>
                    <p className="text-muted-foreground">
                      No test artifacts found. Run tests to populate{' '}
                      <code>test-results/</code>.
                    </p>
                    <p className="text-xs text-muted-foreground mt-2">
                      Expected files:{' '}
                      <code>test-results/e2e-detailed.json</code> or{' '}
                      <code>test-results/e2e.json</code>, and{' '}
                      <code>test-results/unit.json</code>.
                    </p>
                    {error && (
                  ...
                  <p className="text-muted-foreground">
                    Select a test suite to view detailed test information
                  </p>
                )}
                <p className="text-xs text-muted-foreground mt-2">
                  View individual test failures, terminal output, code context,
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex items-center justify-between p-4 border rounded-lg">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Detailed analysis of failing tests and errors
                    </p>
                  </div>
                  <Button variant="outline" size="sm">
                    <FileText className="h-4 w-4 mr-2" />
                ...
                <div className="flex items-center justify-between p-4 border rounded-lg">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Living documentation of all test cases
                    </p>
                  </div>
                  <Button variant="outline" size="sm">
                    <FileText className="h-4 w-4 mr-2" />
